...
- [2. Rust特性分析及改写模块分析](#2. Rust特性分析及改写模块分析)
   -  [2.1. Rust特性分析](#2.1. Rust特性分析)
   -  [2.2. Rust改写各部分可行性分析](#2.2. Rust改写各部分可行性分析)

- [3. liteOS-m内核分析](#3. liteOS-m内核分析)
   - [3.1. 重要的数据结构](#3.1. 重要的数据结构)
   - [3.2. 任务处理](#3.2. 任务处理)
   - [3.3. 内存管理](#3.3. 内存管理)
     - [3.3.1. 静态内存管理](#3.3.1. 静态内存管理)
     - 3.3.2. [动态内存管理](#3.3.2. 动态内存管理)

- [4. Rust开发的可行性](#4. Rust开发的可行性)
   - [4.1. Rust编译器的使用](#4.1. Rust编译器的使用)
   - [4.2. C与Rust交互与互操作性分析](#4.2. C与Rust交互与互操作性分析)
<!-- /vscode-markdown-toc -->
...
![pic](../src/任务就绪队列.png)
#### 3.1.3. 任务排序链表 ```g_taskSortLinkList```
...
###### 3.3.2.2.3. 找到满足大小的内存块```OsMemFreeNodeGet(VOID *pool, UINT32 size)```
...
###### 3.3.2.2.4. 内存块申请```OsMemAlloc(VOID *pool, UINT32 size)```
